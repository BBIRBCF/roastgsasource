...
maindir <- "path2benchmarkarrays/"
resp <- paste0(maindir, "roastgsa_on_benchmarking_data_arrays/")
dir.create(resp)
...
#    kegg.gs <- kegg.gs[which(sapply(kegg.gs,length)>=10&sapply(kegg.gs,length)<500)]
    ...
    RESULTS[[whdata]]  <- lapply(ares, function(x){
            ...
            pv <- sapply(1:1000,function(ds){
                rownames(a2) <- sample(rownames(a1))
                whe  <- which(a2[rownames(mala.kegg[[d2d.map[names(geo2kegg)]]]),]$rank < 50)
                sum((1-a2[rownames(mala.kegg[[d2d.map[names(geo2kegg)]]]),]$rank/nrow(a2))[whe] *
                        mala.kegg[[name1]]$REL.SCORE[whe], na.rm=TRUE)
            })
            M2.pv <- mean(M2<pv)
            c(M1,M2, M1.pv, M2.pv)
...
saveRDS(RESULTS, file = paste0(resp,"/RESULTS.RDS"))




#######  barplots
RESULTS <- readRDS(paste0(resp,"/RESULTS_.RDS"))
...
g <- ggplot(dff1, aes(x=threshold,y=sign.cases, colour= type)) + geom_line(lwd=2) + geom_point(size=3)+theme_classic() + scale_colour_manual(values=cols) +
    geom_vline(xintercept=c(0.05,0.1,0.15,0.25),lty=2, colour="grey")+scale_y_continuous(name ="pvalue < threshold", breaks =c(1:40))
ggsave(g, file = paste0(resp, "sog_arrays.pdf"), width =10,height=7)
...
pdf(paste0(resp,"M1_barplot_arrays.pdf"), width =8, height =8)
...
pdf(paste0(resp,"M2_barplot_arrays.pdf"), width =8, height =8)
layout(cbind(1:8,9:16),widths = c(1,0.1), height = c(rep(1,7),0.5))
par(mar=c(0,1.5,1,0))
for(k in 1:7) {
...
pdf(paste0(resp,"heatmapwithalldata_M2_arrays.pdf"), height =5, width=10)
...
pdf(paste0(resp,"heatmapwithalldata_M1_arrays.pdf"), height =5, width=10)
Heatmap((dad),  col = colorRamp2(c(3, 0, -3), c("blue", "white", "red")),
                heatmap_legend_param = list(title = "centred rank") )
dev.off()
