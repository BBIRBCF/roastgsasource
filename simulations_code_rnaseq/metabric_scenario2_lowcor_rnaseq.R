#################################################################################################
##############  Scenario 2. Simulate new data from GTEX - breast: only a subset- low cor   ######
#################################################################################################
...
for(k in 1:length(Nselall)){
    ...
    ale <- mclapply(1:Nite, function(ite){
        ...
        t1 <- thin_2group(as.matrix(counts.breast),group_prop = 0.5, signal_params = list(mean = 0.6, sd = 0.01),prop_null = 0,alpha=1)
        samps <- c(sample(which(t1$designmat==1),Nselall[k]/2),sample(which(t1$designmat==0),Nselall[k]/2))
        index[[1]] <-  sample(rownames(ynormall2),80)
    ...
    save(power1, avgcor2t, file= paste0(resp, "scenario2_lowcor_rnaseq.Rdata"))
}

load(paste0(resp, "scenario2_lowcor_rnaseq.Rdata"))
nm  <- c("CO.mean", "CO.maxmean", "CO.median", "CO.absmean", "CO.mean.rank", "CO.ksmean", "CO.ksmax")

cat("\n", file = paste0(resp, "scenario2_lowavgcor_rnaseq.txt"), append = FALSE)
