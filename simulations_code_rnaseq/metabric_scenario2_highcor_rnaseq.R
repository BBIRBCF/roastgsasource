#################################################################################################
##############  Scenario 2. Simulate new data from GTEX - breast: only a subset- high cor  ######
#################################################################################################
...
load(paste0(resp,"sc2index.Rdata"))
...
for(k in 1:length(Nselall)){
    ...
    ale <- mclapply(1:Nite, function(ite){
        ...
        t1 <- thin_2group(as.matrix(counts.breast),group_prop = 0.5, signal_params = list(mean = 2.5, sd = 0.01),prop_null = 0,alpha=1)
        samps <- c(sample(which(t1$designmat==1),Nselall[k]/2),sample(which(t1$designmat==0),Nselall[k]/2))
        index2 <- index
        whsub <- sample(1:length(index[[1]]),12)
        ...
        ysim[index2[[1]][whsub],] <- aux[index2[[1]][whsub],samps]
        N  <- ncol(ysim)
        ...
        basemean <- apply(ysim,1,mean)[index2[[1]][whsub]]
        ...
        list(CO.mean, CO.maxmean, CO.median, CO.absmean, CO.mean.rank, CO.ksmean, CO.ksmax, basemean)

    }, mc.cores = mc.cores)

    power1[[1]][[k]] <- lapply(1:length(ale[[1]]),function(o) sapply(ale, function(x) x[[o]][1]))
    basemeans[[1]][[k]] <- lapply(ale, function(x) x[[length(ale[[1]])]])
   save(power1, basemeans, avgcor2t, file= paste0(resp, "scenario2_only_subset_of_genes_rnaseq.Rdata"))
}

load(paste0(resp, "scenario2_only_subset_of_genes_rnaseq.Rdata"))
nm  <- c("CO.mean", "CO.maxmean", "CO.median", "CO.absmean", "CO.mean.rank", "CO.ksmean", "CO.ksmax")

cat("\n", file = paste0(resp, "scenario2_table_rnaseq.txt"), append = FALSE)
...
L=3
wh1 <- which(sapply(basemeans[[1]][[L]],function(x) sum(x<10))==0)
cor(cbind(sapply(basemeans[[1]][[L]],function(x) sum(x<10)),-log10(power1[[1]][[L]][[2]])))
sapply(seq(20,200,by=10),function(R) cor(cbind(sapply(basemeans[[1]][[L]],function(x) sum(x<R))[wh1],-log10(power1[[1]][[L]][[2]])[wh1]), method="spearman")[1,2])



p1 <- unlist(lapply(1:3, function(l) power1[[1]][[l]][[8]]))
df1 <- data.frame(cors=p1, N=as.factor(rep(Nselall[1:3],each=500)))

g1 <- ggplot(df1, aes(x=N, y=cors)) + geom_violin() +theme_classic() +geom_hline(yintercept=0,lty=2,colour="grey") +
