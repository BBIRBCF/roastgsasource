#############################################################################################################
##############  Scenario 3. Simulate new data from Metabric cov: only a subset of genes up and down #########
#############################################################################################################
...
dir.create(resp <- paste0(maindir,"/scenarios_recoveryrates_arrays/"))
...
####### load clusters
clusts <- readRDS(paste0(resp,"clusts_sc3.RDS"))
...
load(paste0(resp, "scenario3_upanddown_effect_onlyfewgenes.Rdata"))
####### Simulations
k <- 1
avgcor2t <- numeric()
#power1  <- list()
...
for(SA in c(1,2,3)){
  ...
  for(k in 1:length(Nselall)){
    ...
    ale <- mclapply(1:Nite, function(ite){
        ...
        mu[,(Nsel/2+1):Nsel] <- rnorm(p, 0, 0.1)
        (dec.clust1  <- sample(clusts[[SA]]$cluster,1))
        dec.clust2  <- dec.clust1[1]
        (dec.clust1  <- sample(clusts[[SA]]$cluster[clusts[[SA]]$cluster!=dec.clust2],1))

        mu[index2[[whind]][clusts[[SA]]$cluster == dec.clust1],(Nsel/2+1):Nsel] <- rnorm(sum(clusts[[SA]]$cluster == dec.clust1), 0.8, 0.1)
        mu[index2[[whind]][clusts[[SA]]$cluster == dec.clust2],(Nsel/2+1):Nsel] <- rnorm(sum(clusts[[SA]]$cluster == dec.clust2), -0.8, 0.1)
    ...
    save(power1, avgcor2t, file= paste0(resp, "scenario3_upanddown_effect_onlyfewgenes.Rdata"))
  }
}
...
cat("\n", file = paste0(resp, "scenario3_table.txt"), append = FALSE)
