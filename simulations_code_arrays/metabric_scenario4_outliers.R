####################################################################################
##############  Scenario 4. Simulate new data from Metabric cov: outliers  #########
####################################################################################
...
load(paste0(resp, "scenario4_outliers.Rdata"))
####### Simulations
k <- 1
avgcor2t <- numeric()
#power1  <- list()
...
for(SA in c(1,2,3)){
  whind <- whindall[SA]
  aux = cov2cor(tcov[index2[[whind]],index2[[whind]]])
...
#  power1[[whind]]  <- list()
  ...
  for(k in 5:length(Nselall)){
    ...
    ale <- mclapply(1:Nite, function(ite){
        ...
        dec.clust1  <- sample(index2[[whind]], min(3,round(length(index2[[whind]])/10)))
        mu[dec.clust1,(Nsel/2+1):Nsel] <- rnorm(length(dec.clust1), 3, 0.1)
    ...
    save(power1, avgcor2t, file= paste0(resp, "scenario4_outliers.Rdata"))
  }
}
...
cat("\n", file = paste0(resp, "scenario4_table.txt"), append = FALSE)
