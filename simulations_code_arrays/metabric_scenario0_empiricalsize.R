#################################################################################################
##############  Scenario 0. Simulate new data from Metabric cov: no effect  #####################
#################################################################################################
...
#  power1[[whind]]  <- list()
  for(k in length(Nselall)){
    ...
    save(power1, avgcor2t, file= paste0(resp, "scenario0_empirical_size.Rdata"))
  }
}


###### Write recovery rates into table
load(paste0(resp, "scenario0_empirical_size.Rdata"))
